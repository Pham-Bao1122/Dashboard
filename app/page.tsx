...
export default function DashboardPage() {
  ...
  const nodeInfoList = data?.NODE_INFO || {}
  
  const floors = Array.from(new Set(Object.values(nodeInfoList).map((info: any) => info.LOCATION || 'Unknown')))
  
  const nodesInActiveFloor = Object.entries(nodeInfoList)
    .filter(([_, info]: [string, any]) => (info.LOCATION || 'Unknown') === activeFloor)
    .map(([id, info]: [string, any]) => ({ id, room: info.NAME || 'Unknown' }))

  const roomsInActiveFloor = Array.from(new Set(nodesInActiveFloor.map(n => n.room)))

  useEffect(() => {
    if (floors.length > 0 && !activeFloor) {
      setActiveFloor(floors[0] as string)
    }
  }, [floors, activeFloor])

  useEffect(() => {
    if (roomsInActiveFloor.length > 0 && (!activeRoom || !roomsInActiveFloor.includes(activeRoom))) {
      setActiveRoom(roomsInActiveFloor[0] as string)
    }
  }, [roomsInActiveFloor, activeRoom])

  useEffect(() => {
    const targetNode = nodesInActiveFloor.find(n => n.room === activeRoom)
    if (targetNode) {
      setActiveNodeId(targetNode.id)
    } else {
      setActiveNodeId('')
    }
  }, [activeRoom, nodesInActiveFloor])
  ...
  const activeSensorData = activeNodeId && data?.NODES ? data.NODES[activeNodeId] : null
      ...
      <div className="space-y-6">
        ...
        {floors.length > 0 ? (
          ...
          <div className="p-4 bg-amber-50 text-amber-600 rounded-lg flex gap-2 items-center dark:bg-amber-950/50 dark:text-amber-400 border border-amber-200/50">
            ...
            <span>Đang tải cấu trúc tòa nhà từ máy chủ hoặc chưa có dữ liệu...</span>
          </div>
        )}
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="mb-4">
              <h2 className="text-xl font-bold flex items-center gap-2">
                ...
                <span className="text-sm font-normal text-muted-foreground ml-2">(Trạm: {activeNodeId})</span>
              </h2>
            </div>

            {activeSensorData && activeSensorData.NODE_STATE === 1 ? (
              <>
                <MonitoringCards data={activeSensorData as any} loading={loading} />
                
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                  <div className="md:col-span-2 space-y-6">
                    <DeviceStatusCards data={activeSensorData as any} loading={loading} />
              ...
              <div className="p-12 text-center border-2 border-dashed border-border rounded-xl bg-muted/10 mt-6">
                ...
                <p className="text-muted-foreground max-w-sm mx-auto">
                  Trạm {activeNodeId} tại khu vực này chưa được cấp nguồn hoặc mất kết nối mạng.
                </p>
              </div>
            )}
          </div>
